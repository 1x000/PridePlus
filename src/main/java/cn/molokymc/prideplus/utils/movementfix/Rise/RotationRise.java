...
public class RotationRise {
    ...
    public static cn.molokymc.prideplus.utils.math.Vector2f applySensitivityPatchA(final cn.molokymc.prideplus.utils.math.Vector2f rotation, final cn.molokymc.prideplus.utils.math.Vector2f previousRotation) {
        final float mouseSensitivity = (float) (mc.gameSettings.mouseSensitivity * (1 + Math.random() / 10000000) * 0.6F + 0.2F);
        final double multiplier = mouseSensitivity * mouseSensitivity * mouseSensitivity * 8.0F * 0.15D;
        final float yaw = previousRotation.x + (float) (Math.round((rotation.x - previousRotation.x) / multiplier) * multiplier);
        final float pitch = previousRotation.y + (float) (Math.round((rotation.y - previousRotation.y) / multiplier) * multiplier);
        return new cn.molokymc.prideplus.utils.math.Vector2f(yaw, MathHelper.clamp_float(pitch, -90, 90));
    }
    ...
    public static cn.molokymc.prideplus.utils.math.Vector2f smooth(final cn.molokymc.prideplus.utils.math.Vector2f lastRotation, final cn.molokymc.prideplus.utils.math.Vector2f targetRotation, final double speed) {
        float yaw = targetRotation.x;
        float pitch = targetRotation.y;
        ...
        return new cn.molokymc.prideplus.utils.math.Vector2f(yaw, pitch);
    }
