package dev.stable.ui.clickguis.compact.impl;
...
public class SettingComponents implements Screen {
    ...
    public void handle(int mouseX, int mouseY, int button, GuiEvents type) {
        ...
        for (Setting setting : module.getSettingsList()) {
            ...
            float middleSettingY = (float) (MathUtils.roundToHalf(y + rubikFont16.getMiddleOfBox(settingHeight) + (count * settingHeight)));

            if (setting instanceof NumberSetting) {
                NumberSetting numberSetting = (NumberSetting) setting;
                rubikFont16.drawString(setting.name, x + 5, middleSettingY, -1);
            ...
            if (setting instanceof ModeSetting) {
                ...
                float modeRectWidth = (float) rubikFont14.getStringWidth(StringUtils.getLongestModeName(modeSetting.modes)) + 10;
                ...
                if (!openAnimation.isDone() || openAnimation.getDirection().equals(Direction.FORWARDS)) {
                    ...
                    for (String mode : modeSetting.modes) {
                        ...
                        GlStateManager.color(1, 1, 1, 1);
                        Gui.drawRect2(x + rectWidth - (modeRectWidth + 5), modeY - rubikFont14.getMiddleOfBox(modeSize), modeRectWidth, modeSize,
            ...
            if (setting instanceof MultipleBoolSetting) {
                ...
                if (!openingAnimation.isDone() || openingAnimation.getDirection().equals(Direction.FORWARDS)) {
                    ...
                    for (BooleanSetting booleanSetting : booleanSettings) {
                        ...
                        RenderUtil.resetColor();
                        Gui.drawRect2(x + rectWidth - (width + 5), boolSettingY - rubikFont14.getMiddleOfBox(boolRectSize),
