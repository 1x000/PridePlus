...
import dev.stable.ui.altmanager.helpers.AltManagerUtils;
    ...
    public static class AltRect implements Screen {
        ...
        public void drawScreen(int mouseX, int mouseY) {
            ...
            if (alt.username != null) {
                usernameText = Utils.tenacityBoldFont16.trimStringToWidth(alt.username, (int) (width - (altSize + 10)));
            }
            float totalHeight = Utils.tenacityBoldFont16.getHeight() + 6 + Utils.rubikFont14.getHeight();
            ...
            if (alt.hypixelBan != null) {
                ...
                if (diff > 0 || ban.getUnbanDate() == 0) {
                    hasBan = true;
                    totalHeight += Utils.tenacityBoldFont14.getHeight() + 4;
                }
            }
            ...
            Utils.tenacityBoldFont16.drawString(usernameText, x + 3 + altSize + 3.5f, usernameY, textColor);

            float typeX = x + 7 + altSize + Utils.tenacityBoldFont16.getStringWidth(usernameText);
            float size = Utils.tenacityBoldFont16.getHeight() + 2;
            switch (alt.getType()) {
                case "Microsoft":
                ...
                case "Mojang":
                    ...
                    Utils.mc.getTextureManager().bindTexture(new ResourceLocation("Pride/mojang.png"));
                ...
                case "Cracked":
                    ...
                    break;
                case "Not logged in":
                    Utils.tenacityBoldFont14.drawString("§bNot logged in", typeX, usernameY + 1, ColorUtil.applyOpacity(-1, 1 - credsAnim));
                    break;
            }

            drawBan(x + altSize + 6.5f, usernameY + Utils.tenacityBoldFont16.getHeight() + 6, 1 - credsAnim);
            if (hasBan) {
                usernameY += Utils.tenacityBoldFont14.getHeight() + 6;
            }
            ...
            hoveringCreds = HoveringUtil.isHovering(x + 3 + altSize + 1.5f, (usernameY + Utils.tenacityBoldFont16.getHeight() + 6) - 2,
                    Utils.tenacityBoldFont14.getStringWidth("§n" + credsText) + 4, Utils.tenacityBoldFont14.getHeight() + 4, mouseX, mouseY);

            float opacity = showCreds ? (hoveringCreds ? .75f : 0.5f) : hoveringCreds ? .5f : .35f;
            Utils.tenacityBoldFont14.drawString("§n" + credsText, x + 3 + altSize + 3.5f,
                    usernameY + Utils.tenacityBoldFont16.getHeight() + 6, ColorUtil.applyOpacity(-1, opacity));
            ...
            if (showCreds || !credsAnimation.isDone()) {
                ...
                if (alt.email != null) {
                    Utils.tenacityBoldFont14.drawString("Email: " + alt.email, xVal, yVal, ColorUtil.applyOpacity(-1, credsAnim));
                    yVal += Utils.tenacityBoldFont14.getHeight() + 4;
                }

                Utils.tenacityBoldFont14.drawString("Password: " + alt.password, xVal, yVal, ColorUtil.applyOpacity(-1, credsAnim));
                RenderUtil.scaleEnd();
            }
            ...
            if (Client.INSTANCE.getAltManager().currentSessionAlt == alt) {
                ...
                if (loadingAltRect == this) {
                    float iconWidth = Utils.iconFont16.getStringWidth(FontUtil.REFRESH);
                    float iconHeight = Utils.iconFont16.getHeight();
                    ...
                    Utils.iconFont16.drawString(FontUtil.REFRESH, iconX, iconY, ColorUtil.applyOpacity(-1, .5f));
                    RenderUtil.rotateEnd();
                } else {
                    Alt.AltState altState = alt.altState;
                    if (altState == null) return;
                    String text = altState.getIcon();
                    Utils.iconFont16.drawString(text, x + width - (Utils.iconFont16.getStringWidth(text) + 5), y + height - (Utils.iconFont16.getHeight() + 5),
        ...
        public void drawBan(float x, float y, float alpha) {
            if (alt.hypixelBan != null) {
                ...
                if (diff > 0) {
                    ...
                    Utils.tenacityBoldFont14.drawString("§lBanned §ron Hypixel for §l" + str, x, y, ColorUtil.applyOpacity(-1, alpha * .5f));
                } else if (ban.getUnbanDate() == 0) {
                    Utils.tenacityBoldFont14.drawString("§lBanned §ron Hypixel permanently", x, y, ColorUtil.applyOpacity(-1, alpha * .5f));
                }
            }
        }
    }
