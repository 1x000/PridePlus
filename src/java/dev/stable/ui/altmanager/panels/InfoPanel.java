...
public class InfoPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        Utils.tenacityBoldFont32.drawCenteredString("Information", getX() + getWidth() / 2f, getY() + 3, ColorUtil.applyOpacity(-1, .75f));

        float controlY = getY() + Utils.tenacityBoldFont32.getHeight() + 8;
        for (Pair<String, String> control : controlInfo) {
            Utils.tenacityBoldFont18.drawString(control.getFirst() + " -", getX() + 12, controlY, ColorUtil.applyOpacity(-1, .5f));
            Utils.rubikFont18.drawString(control.getSecond(), getX() +
                    Utils.tenacityBoldFont18.getStringWidth(control.getFirst() + " -") + 14, controlY, ColorUtil.applyOpacity(-1, .35f));

            controlY += Utils.tenacityBoldFont18.getHeight() + 6;
        }

        String text = "Combos must be formatted in the following format: ";
        String text2 = "email:password";
        float textWidth = Utils.rubikFont18.getStringWidth(text);
        float text2Width = Utils.tenacityBoldFont18.getStringWidth(text2);
        float middleX = getX() + getWidth() / 2f - (textWidth + text2Width) / 2f;

        Utils.rubikFont18.drawString(text, middleX, controlY + 4, ColorUtil.applyOpacity(-1, .5f));
        Utils.tenacityBoldFont18.drawString(text2, middleX + textWidth, controlY + 4, ColorUtil.applyOpacity(-1, .5f));

        Utils.rubikFont18.drawCenteredString("Combo lists must have a new line seperating each combo",
