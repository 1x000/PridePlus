...
import dev.stable.utils.misc.Rotation;
import dev.stable.utils.Utils;
import dev.stable.utils.misc.MathUtils;
import dev.stable.utils.movementfix.Rise.Location;
...
import static dev.stable.utils.movementfix.Rise.RotationUtils.getNearestPointBB;

public class RotationUtils implements Utils {
    public static Vector2f calculate(final Vec3 to) {
        return calculate(mc.thePlayer.getCustomPositionVector().add(0, mc.thePlayer.getEyeHeight(), 0), new dev.stable.event.impl.player.Vector3d(to.xCoord, to.yCoord, to.zCoord));
    }
    public static Vector2f calculate(final dev.stable.event.impl.player.Vector3d position, final EnumFacing enumFacing) {
        ...
        return calculate(new dev.stable.event.impl.player.Vector3d(x, y, z));
    }

    public static Vector2f calculate(final dev.stable.event.impl.player.Vector3d to) {
        return calculate(mc.thePlayer.getCustomPositionVector().add(0, mc.thePlayer.getEyeHeight(), 0), to);
    }
    public static Vector2f calculate(final dev.stable.event.impl.player.Vector3d from, final dev.stable.event.impl.player.Vector3d to) {
        final dev.stable.event.impl.player.Vector3d diff = to.subtract(from);
