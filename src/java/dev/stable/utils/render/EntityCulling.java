...
import dev.stable.event.impl.game.RenderTickEvent;
...
public class EntityCulling extends Module {
    ...
    public EntityCulling() {
        super("EntityCulling","EntityCulling", Category.RENDER, "Culls entities that are out of range");
    ...
    private static final RenderManager renderManager = Utils.mc.getRenderManager();
    ...
    public void onRendererLivingEntityEvent(RendererLivingEntityEvent e) {
        ...
        if (entity == Utils.mc.thePlayer || entity.worldObj != Utils.mc.thePlayer.worldObj ||
               /* (armorstand && ((EntityArmorStand) entity).hasMarker()) ||*/
                (entity.isInvisibleToPlayer(Utils.mc.thePlayer))) {
            return;
        }
...
//        EntityLivingBase entity = e.getEntity();
        ...
        if (EntityCulling.shouldPerformCulling) {
            final float entityDistance = entity.getDistanceToEntity(Utils.mc.thePlayer);
    ...
    public void onRenderTickEvent(RenderTickEvent e) {
        if (e.isPre()) {
            Utils.mc.addScheduledTask(this::check);
        }
    }
    ...
    public static boolean canRenderName(EntityLivingBase entity) {
        ...
        return Minecraft.isGuiEnabled()
                && entity != Utils.mc.getRenderManager().livingPlayer
    ...
    public void onTickEvent(TickEvent e) {
        ...
        this.destroyTimer = 0;
        WorldClient theWorld = Utils.mc.theWorld;
        if (theWorld == null) {
            return;
        }
