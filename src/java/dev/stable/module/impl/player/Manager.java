...
import dev.stable.module.impl.render.CameraClip;
...
public class Manager extends Module {
    ...
    private final NumberSetting delay = new NumberSetting("Delay", 1.0, 20, 0, 1);
    ...
    public Manager() {
        super("Cleaner","背包整理", Category.PLAYER, "cleans up your inventory");
    ...
    public void onMotionEvent(MotionEvent e) {
        if(test == 0){
            setSuffix("Tick:"+String.valueOf(test));
        }else{
            setSuffix("Ticks:"+String.valueOf(test));
        }

        if(HUDMod.calculateBPS()>=5.00){
            test += 5;
        }if(HUDMod.calculateBPS()>=4.50&&HUDMod.calculateBPS()<5.00) {
            test += 4;
        }if(HUDMod.calculateBPS()>=4.00&&HUDMod.calculateBPS()<4.50){
            test += 3;
        }if(HUDMod.calculateBPS()>=2.00&&HUDMod.calculateBPS()<4.00&&test>0){
            test -= 3;
        }if(HUDMod.calculateBPS()>1.00&&HUDMod.calculateBPS()<2.00&&test>0) {
            test -= 5;
        }if(HUDMod.calculateBPS()<1.00&&test>0) {
            test -= 7;
        }if(test<0){
            test = 0;
        }
        if (e.isPost() || canContinue()) return;
        if (!Utils.mc.thePlayer.isUsingItem() && (Utils.mc.currentScreen == null || Utils.mc.currentScreen instanceof GuiChat || Utils.mc.currentScreen instanceof GuiInventory || Utils.mc.currentScreen instanceof GuiIngameMenu)) {
            if (isReady()) {
        ...
        if (stealing) {
            ...
            if (item instanceof ItemArmor) {
                for (int type = 1; type < 5; type++) {
                    ItemStack is = Utils.mc.thePlayer.inventoryContainer.getSlot(type + 4).getStack();
                    if (is != null) {
                        String typeStr = "";
                        switch (type) {
                            case 1:
        ...
        for (int i = 9; i < 45; i++) {
            ...
            if (slot.getHasStack()) {
                ...
                if (isBestPickaxe(is) && !isBestWeapon(is)) {
                    ...
                    Slot slot2 = Utils.mc.thePlayer.inventoryContainer.getSlot(desiredSlot);
    ...
    private void moveArrows() {
        ...
        for (int i = 36; i < 45; i++) {
            ...
            if (is != null && is.getItem() == Items.arrow) {
                for (int j = 0; j < 36; j++) {
                    if (Utils.mc.thePlayer.inventoryContainer.getSlot(j).getStack() == null) {
                        fakeOpen();
    ...
    private void swapBlocks() {
        ...
        if (mostBlocksSlot != -1 && mostBlocksSlot != desiredSlot) {
            ...
            Slot dss = Utils.mc.thePlayer.inventoryContainer.getSlot(desiredSlot);
            ItemStack dsis = dss.getStack();
            if (!(dsis != null && dsis.getItem() instanceof ItemBlock && dsis.stackSize >= Utils.mc.thePlayer.inventoryContainer.getSlot(mostBlocksSlot).getStack().stackSize && Arrays.stream(serverItems).noneMatch(dsis.getDisplayName().toLowerCase()::contains))) {
    ...
    private void fakeClose() {
        if (isInvOpen) {
            if (!inventoryOnly.isEnabled() && inventoryPackets.isEnabled())
                PacketUtils.sendPacketNoEvent(new C0DPacketCloseWindow(Utils.mc.thePlayer.inventoryContainer.windowId));
            isInvOpen = false;
        }
    }
    ...
    private boolean canContinue() {
        return (inventoryOnly.isEnabled() && !(Utils.mc.currentScreen instanceof GuiInventory)) || (onlyWhileNotMoving.isEnabled() && MovementUtils.isMoving());
    }
    ...
    private enum ItemType {
        ...
        public Slot getSlot() {
            return Utils.mc.thePlayer.inventoryContainer.getSlot(getDesiredSlot());
        }
