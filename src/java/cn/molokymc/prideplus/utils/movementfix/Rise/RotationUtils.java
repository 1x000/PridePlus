...
public final class RotationUtils {
	...
	public static VecRotation lockView(final AxisAlignedBB bb, final boolean outborder, final boolean random,
									   final boolean predict, final boolean throughWalls, final float distance) {
		if (outborder) {
			final cn.molokymc.prideplus.utils.movementfix.Rise.Vec3 vec3 = new Vec3(bb.minX + (bb.maxX - bb.minX) * (x * 0.3 + 1.0), bb.minY + (bb.maxY - bb.minY) * (y * 0.3 + 1.0), bb.minZ + (bb.maxZ - bb.minZ) * (z * 0.3 + 1.0));
			return new VecRotation(vec3, toRotation(vec3, predict));
		}

		final cn.molokymc.prideplus.utils.movementfix.Rise.Vec3 randomVec = new Vec3(bb.minX + (bb.maxX - bb.minX) * x * 0.8, bb.minY + (bb.maxY - bb.minY) * y * 0.8, bb.minZ + (bb.maxZ - bb.minZ) * z * 0.8);
		final Rotation randomRotation = toRotation(randomVec, predict);

		final cn.molokymc.prideplus.utils.movementfix.Rise.Vec3  e = mc.thePlayer.getPositionEyes2(1F);
